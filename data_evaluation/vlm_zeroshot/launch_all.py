...
EXPECTED_CALLS_PER_MODEL = 1428 * 5  # 1,428 tasks x (full + blind + 3 shuffles)


def build_command(model_key: str, extra: list) -> list:
    return [gpu_allocation.python_for(model_key),
            os.path.join(_HERE, "run_answerer.py"),
            "--model", model_key, "--no-echo", *extra]


def launch(model_keys: list, extra: list, dry_run: bool = False) -> dict:
    ...
    for model_key in model_keys:
        command = build_command(model_key, extra)
        if dry_run:
        ...
        stdout_path = os.path.join(LOG_DIR, f"{model_key}_launch.out")
    ...
    if not dry_run:
        state_path = os.path.join(LOG_DIR, "launch_state.json")
        with open(state_path, "w") as f:
            json.dump({"started": time.time(), "pids": processes}, f, indent=1)
        print(f"\nstate: {state_path}")
        print("progress:  python launch_all.py --status")
        print("follow:    tail -f logs/<model>_mcq_*.log")
    return processes
...
def status() -> None:
    ...
    for model_key in gpu_allocation.MODEL_GPU:
        path = checkpoint_path(model_key, "mcq")
        records = 0
        ...
        if pid:
            try:
                ...
                alive = "no"
        pct = 100 * records / EXPECTED_CALLS_PER_MODEL
        print(f"{model_key:12s} {str(pid or '-'):>8s} {alive:>6s} {records:>9d} {pct:8.1f}%")

    print(f"\nexpected {EXPECTED_CALLS_PER_MODEL} calls/model "
          f"(1,428 tasks x 5 conditions)")
...
def main() -> None:
    ...
    launch(args.models, extra, dry_run=args.dry_run)
